...
func loadConfiguration(args *CommandLineArgs) (*ini.File, error) {
	...
	// load default overrides 命令行默认参数覆盖default.ini
	...
	// apply environment overrides 环境参数覆盖default.ini
	err = applyEnvVariableOverrides(parsedFile)
	if err != nil {
		return nil, err
	}

	// apply command line overrides 命令行添加参数覆盖default.ini
